...
# Modified Reddit initialization
try:
    reddit = praw.Reddit(
        client_id=CLIENT_ID,
        ...
        trophy_kind=None,
        oauth_url=None,
        ratelimit_seconds=30,  # Changed from None to 30 seconds
        reddit_url=None,
        timeout=30
    )
...
bot = commands.Bot(
    command_prefix="/", 
    intents=intents,
    chunk_guilds_at_startup=False,  # Disable guild chunking at startup
    heartbeat_timeout=60.0,        # Reduce heartbeat timeout
    guild_ready_timeout=2.0,        # Reduce guild ready timeout
    assume_unsync_clock=True,        # Better time sync
...
# Modified get_meme function to handle Reddit initialization failure
...
@bot.event
async def on_ready():
    print(f"\nBot is ready as {bot.user.name}")
    server_count = len(bot.guilds)
    print(f"\nThe bot is in {server_count:,} servers.")
    ...
    try:
        ...
        if not last_sync_time or (current_time - last_sync_time).total_seconds() > SYNC_COOLDOWN:
            ...
            synced = await bot.tree.sync(guild=None)  # Sync globally
            last_sync_time = current_time
            print(f"\nSynced {len(synced)} command(s) in {time.time() - start_time:.2f} seconds")
        else:
            print("\nSkipping command sync due to cooldown")
    except Exception as e:
...
async def meme_search(interaction: discord.Interaction, keyword: str):
    try:
        ...
        if not keyword or not isinstance(keyword, str):
            await interaction.followup.send("Please provide a valid keyword for the search.", ephemeral=True)
            return
...
async def invite(interaction: discord.Interaction):
    ...
    support_button = Button(
        ...
        style=discord.ButtonStyle.link,
        url="https://discord.gg/TpD8qT5z54",
        emoji="❓"
    )
...
@bot.tree.command(name="ping", description="Check the bot's latency and connection quality.")
async def ping(interaction: discord.Interaction):
    try:
        # Get initial websocket latency
        ws_latency = round(bot.latency * 1000)
        
        # Measure API latency with minimal overhead
        ...
        def get_quality_info(latency):
            if latency < 100:
                return "Excellent", "🟢", discord.Color.green()
            elif latency < 200:
                return "Good", "🟡", discord.Color.yellow()
            elif latency < 300:
                return "Fair", "🟠", discord.Color.orange()
            else:
                return "Poor", "🔴", discord.Color.red()

        ws_quality, ws_emoji, color = get_quality_info(ws_latency)
        api_quality, api_emoji, _ = get_quality_info(api_latency)
        ...
        embed.add_field(
            ...
            value=f"{ws_emoji} `{ws_latency}ms` ({ws_quality})",
            inline=False
        )
        embed.add_field(
            name="API Latency",
            value=f"{api_emoji} `{api_latency}ms` ({api_quality})",
            inline=False
        )

        # Add connection tips if latency is high
        if ws_latency > 200 or api_latency > 200:
            tips = (
                "**Tips to improve latency:**\n"
                "• Check your internet connection\n"
                "• Close unnecessary applications\n"
                "• Try a different Discord server region\n"
                "• Consider using a wired connection"
            )
            embed.add_field(name="Optimization Tips", value=tips, inline=False)
...
async def report(interaction: discord.Interaction, issue: str):
    ...
    try:
        ...
        if support_channel:
            # Check if the bot has permission to send messages in the support channel
            if not support_channel.permissions_for(support_channel.guild.me).send_messages:
                await interaction.response.send_message(
                    "I don't have permission to send messages in the support channel.",
                    ephemeral=True
                )
                return
            ...
            async def acknowledge_callback(button_interaction: discord.Interaction):
                ...
                # Notify the user who made the report
                try:
                    await interaction.user.send("Your report has been acknowledged.")
                except discord.Forbidden:
                    print(f"Could not send DM to {interaction.user.name}. They may have DMs disabled.")
            ...
            async def resolve_callback(button_interaction: discord.Interaction):
                ...
                # Notify the user who made the report
                try:
                    await interaction.user.send("Your report has been resolved.")
                except discord.Forbidden:
                    print(f"Could not send DM to {interaction.user.name}. They may have DMs disabled.")
...
def run_bot():
    try:
        ...
        input("Press Enter to exit...") # Add this line to keep console open
        sys.exit(1)
